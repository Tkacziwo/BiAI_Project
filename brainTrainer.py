...
class FilteredBrainTrainer():
            ...
            if img_tensor != None and ready_expected_result != [0,0,0]:
                ...
                for i in range(10):
                    ...
                    if cnn_result.shape == ready_expected_result.view(1, -1).shape:
                        loss = self.loss_function(cnn_result, ready_expected_result.view(1,-1))
                        loss.backward()
                        self.optimizer.step()
    ...
    def train_brain(self, epochs_num: int):
        ...
        for j in range(1, 6):
            ...
            self.brain = CNN_Brain.CNN(j).to(self.device)

            for i in range(epochs_num):
                self.brain.train(True)
                loss_for_epoch = self.epoch_train(number_of_colors=j)
                self.brain.eval()
                torch.no_grad()
                print("Epoch #{}. Loss: {}".format(i+1,loss_for_epoch))
                if loss_for_epoch < best_vloss:
                    best_vloss = loss_for_epoch
                    brain_filename = 'models/filtered_brain_color_{}_{}.pt'.format(j, (i+1))
